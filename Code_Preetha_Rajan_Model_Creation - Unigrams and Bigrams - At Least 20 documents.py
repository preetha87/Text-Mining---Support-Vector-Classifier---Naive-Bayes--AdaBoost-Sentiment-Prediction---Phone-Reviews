...
def main():
    ...
    df.to_csv('Cleaned_UserReviews_Unigrams_Bigrams.csv', encoding='utf-8', index=False)
    #### Reading cleaned reviews as dataframe
    cleaned_data = pd.read_csv("Cleaned_UserReviews_Unigrams_Bigrams.csv", encoding = "ISO-8859-1")
    ...
    pd.DataFrame.from_dict(data=dict([word, i] for i, word in enumerate(tfidf.get_feature_names())), orient='index').to_csv('vocabulary - unigrams and bigrams.csv', header=False)
    ...
    if SVC_mean_accuracy > NBC_mean_accuracy:
        ...
        joblib.dump(clf, 'svc-unigrams and bigrams.sav')
    else:
        clf = MultinomialNB().fit(data, targets)
        joblib.dump(clf, 'nbc-unigrams and bigrams.sav')
